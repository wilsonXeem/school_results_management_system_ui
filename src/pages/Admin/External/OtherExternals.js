  ...
  useEffect(() => {
    ...
    fetch("http://127.0.0.1:1234/api/sessions/")
      ...
      .then((json) => {
        ...
        setExternals(
          json.sessions.find((session) => session.session === sesion).externals
        );
        setOther_sessions(
              ...
              <div>
                {first_semester[i].map((course_code) => (
                  <button
                    ...
                    onClick={() =>
                      navigate(
                        `/admin/faculty/${sesion}/${lev}/1/${course_code}`
                      )
                    }
                  >
              ...
              <div>
                {second_semester[i].map((course_code) => (
                  <button
                    ...
                    onClick={() =>
                      navigate(
                        `/admin/faculty/${sesion}/${lev}/2/${course_code}`
                      )
                    }
                  >
                    {course_code} <OpenInNewIcon />
